...
// Cloned from UcEmpMap. Colours by % of 16-64 residents claiming (native DWP proportion).
interface Props {
  wards: ClaimantWard[];
  onSelect: (code: string) => void;
}

export default function ClaimantMap({ wards, onSelect }: Props) {
      ...
      try {
        ...
        L.geoJSON(geo, {
          style: feat => {
            ...
            return { fillColor: w ? rampColor(w.pct_16_64) : '#ccc', fillOpacity: 0.7, color: '#f5f3ee', weight: 1.2 };
          },
          onEachFeature: (feat, layer) => {
            const code = feat.properties?.WD22CD ?? '';
            const w = wardIndex[code];
            if (w) {
              layer.bindTooltip(`<b>${w.ward_name}</b><br>${w.pct_16_64}% of 16–64 residents claiming<br>${w.count.toLocaleString()} claimants`, { sticky: true });
